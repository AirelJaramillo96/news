<template>
    <nav class="sticky inset-0 z-10 block h-max w-full max-w-full rounded-none border border-white/80 bg-white bg-opacity-80 py-2 px-4 text-white shadow-md backdrop-blur-2xl backdrop-saturate-200 lg:px-8 lg:py-4">
        <div class="flex items-center text-gray-900">
            ...
            <div class="ml-20 relative flex h-10 w-full min-w-[200px] max-w-[24rem]">
                <input
                    type="email"
                    v-model="searchQuery"
            ...
            <ul class="ml-auto mr-8 hidden items-center gap-6 lg:flex">
                ...
                <li v-for="category in categories"  class="block p-1 font-sans text-sm font-normal leading-normal text-inherit antialiased">
                    <a @click="serchNews(category.name)" class="flex items-center" href="#">
                        {{category.name}}
                    </a>
                </li>
            </ul>
